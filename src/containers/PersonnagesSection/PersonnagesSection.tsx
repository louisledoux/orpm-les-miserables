...
function PersonnagesSection({
  ...
  return (
    <div className={`flex justify-between items-center px-20px lg:px-120px mb-60px lg:${reverse ? 'flex-row-reverse' : 'flex-row'} ${reverse ? 'flex-col' : 'flex-col'}`}>
      <div className={`w-full lg:max-w-33 h-353px mb-30px lg:mb-0 lg:h-full lg:${reverse ? 'ml-30px' : 'mr-30px'}`}>
        <Image className="object-cover object-top lg:object-center h-full w-full lg:max-h-550" src={image.image} alt={image.alt} style={image.style} />
      </div>
      {pages.map((page) => (
        <div
          key={page.title}
          className={`flex flex-col lg:max-w-66 w-full lg:py-60px px-20px lg:px-40px lg:${reverse ? 'mr-30px' : 'ml-30px'}`}
          style={page.style}
        >
